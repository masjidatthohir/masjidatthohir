document.addEventListener('DOMContentLoaded', function() {
        ...
        uploadButton.addEventListener('click', function() {
            ...
            if (files.length > 0) {
                ...
                for (let i = 0; i < files.length; i++) {
                    const file = files[i];
                    const storageRef = storage.ref('peresmian/' + file.name);
                        ...
                        uploadTask.on('state_changed',
                            ...
                            function() {
                                uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
                                    ...
                                    database.ref('peresmian').push(photoData).then(resolve).catch(reject);
                                });
                            }
                        );
                    });
    ...
    function loadGallery() {
        database.ref('peresmian').once('value').then(function(snapshot) {
            snapshot.forEach(function(childSnapshot) {
                const peresmian = childSnapshot.val();
                const img = document.createElement('img');
                img.src = peresmian.url;
                img.dataset.key = childSnapshot.key; // Menyimpan key di data attribute
                img.addEventListener('click', function() {
                    openPhotoDetails(peresmian);
                });
