document.addEventListener('DOMContentLoaded', function() {
        ...
        uploadButton.addEventListener('click', function() {
            ...
            if (files.length > 0) {
                ...
                for (let i = 0; i < files.length; i++) {
                    const file = files[i];
                    const storageRef = storage.ref('idul_adha/' + file.name);
                        ...
                        uploadTask.on('state_changed',
                            ...
                            function() {
                                uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
                                    ...
                                    database.ref('idul_adha').push(photoData).then(resolve).catch(reject);
                                });
                            }
                        );
                    });
    ...
    function loadGallery() {
        database.ref('idul_adha').once('value').then(function(snapshot) {
            snapshot.forEach(function(childSnapshot) {
                const idul_adha = childSnapshot.val();
                const img = document.createElement('img');
                img.src = idul_adha.url;
                img.dataset.key = childSnapshot.key; // Menyimpan key di data attribute
                img.addEventListener('click', function() {
                    openPhotoDetails(idul_adha);
                });
